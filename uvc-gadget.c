...
/* Tiny valid JPEG (1x1 pixel) as a safe fallback for dummy MJPEG mode. */
static const unsigned char tiny_jpeg[] = {
    ...
    0x01, 0x01, 0x11, 0x00, 0xff, 0xc4, 0x00, 0x14, 0x00, 0x01, 0x00, 0x01, 0x05, 0x01, 0x01, 0x01,
    0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0xff, 0xda,
    0x00, 0x08, 0x01, 0x01, 0x00, 0x00, 0x3f, 0x00, 0xbf, 0xff, 0xd9};
...
int main(int argc, char *argv[])
{
    ...
    if (dummy_data_gen_mode || mjpeg_image) {
        ...
        if (mjpeg_image == NULL && udev->fcc == V4L2_PIX_FMT_MJPEG) {
            udev->mjpeg_video = (void *)tiny_jpeg;
            udev->mjpeg_video_size = sizeof(tiny_jpeg);
            udev->mjpeg_frames = calloc(1, sizeof(*udev->mjpeg_frames));
            if (udev->mjpeg_frames) {
                udev->mjpeg_frames[0].offset = 0;
                udev->mjpeg_frames[0].size = sizeof(tiny_jpeg);
                udev->mjpeg_frame_count = 1;
                udev->imgdata = (void *)tiny_jpeg;
                udev->imgsize = sizeof(tiny_jpeg);
                printf("UVC: using fallback tiny MJPEG frame (%zu bytes)\n", sizeof(tiny_jpeg));
            }
        }
    }
